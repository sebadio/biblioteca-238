  ...
  for (let libro of arrayLibros) {
    ...
    if (!(libro.paginas < min) && !(libro.paginas > max)) {
      /* ((min == undefined && max == undefined) || (libro.paginas >= min && libro.paginas <= max) ||
        (libro.paginas >= min && max == undefined) || (libro.paginas <= max && min == undefined)) */ if (
        search == undefined ||
        search == "" ||
        libro.titulo.toLowerCase().indexOf(search.toLowerCase()) > -1
      ) {
        let contenido = `
                <li
                class="list-group-item bg-dark text-white border-secondary" 
                onclick="redireccionar(${libro.id})">
